...
if(isset($_GET['cha'])){
    ...
    header('location: /pharmacist/employees/add.php');
}
...
$insert = "INSERT INTO `employees` VALUES(null,'$name','$email','$password',3)";
$i = mysqli_query($conn , $insert);
testMessage($i, "Insert Employees");
}
}
...
if (isset($_POST['update'])) {
    ...
    $email = $_POST['email'];
    $password = $_POST['password'];
    $update= "UPDATE `employees` SET `name` = $name,`email` = $email, `password` = $password where `id` = $id";
    $u= mysqli_query($conn , $update);
...
?>
<?php if($noc == '1') : ?>
<a href="/pharmacist/employees/add.php?cha=2" name="cha" class="btn btn-dark">Dark mood</a>
<?php else : ?>
<a href="/pharmacist/employees/add.php?cha=1" name="cha" class="btn btn-light">Light mood</a>
<?php endif ; ?>
    <div class="home">
    <?php if($update):?>
    <h1 class="display-1 text-center text-info">Update Employees</h1>
    <?php else :?>
    <h1 class="display-1 text-center text-info">Add Employees</h1>
    <?php endif; ?>
  </div>
